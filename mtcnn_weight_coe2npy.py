...
def file2list(sram_list):
    change_sram_list = []
    for idx in range(len(sram_list)):
        foo = [float(x) for x in sram_list[idx].split()]
        change_sram_list.append(foo)
    return change_sram_list


def file2list_reverse(sram_list):
    change_sram_list = []
    for idx in range(len(sram_list)):
        foo = [float(np.int8(int(x, 16))) for x in sram_list[idx].split()]
        foo.reverse()
        change_sram_list.append(foo)
    return change_sram_list
...
if __name__ == '__main__':
    ...
    sram4_path = '1201_quant/sram4_quant.txt'
    sram5_path = '1201_quant/sram5_quant.txt'
    sram6_path = '1201_quant/sram6_quant.txt'
    ...
    change_sram4_list = np.array(file2list_reverse(sram4_list[-2967:])) / 64
    change_sram5_list = np.array(file2list_reverse(sram5_list[-4608:])) / 64
    change_sram6_list = np.array(file2list_reverse(sram6_list[-43:])) / 64
    print(np.shape(change_sram4_list))
    print(change_sram4_list[0])

    # pnet_conv1 = int2float(change_sram4_list[:14], pnet_conv1_sh)
    pnet_conv1 = change_sram4_list[:14]
    ...
    pnet_conv2 = change_sram4_list[14:86]
    ...
    pnet_conv3 = change_sram4_list[86:374]
    pnet_conv3 = np.array(pnet_conv3).flatten() / 2
    ...
    pnet_conv4 = change_sram4_list[374:378]
    pnet_conv4 = np.array(pnet_conv4).flatten() / 2
    ...
    pnet_conv5 = change_sram4_list[378:386]
    pnet_conv5 = np.array(pnet_conv5).flatten() / 8
    ...
    rnet_conv1 = change_sram4_list[386:435]
    rnet_conv1 = np.array(rnet_conv1).flatten() / 2
    ...
    rnet_conv2 = change_sram4_list[435:1191]
    rnet_conv2 = np.array(rnet_conv2).flatten() / 4
    ...
    rnet_conv3 = change_sram4_list[1191:2919]
    rnet_conv3 = np.array(rnet_conv3).flatten() / 4
    ...
    pnet_bias2 = change_sram6_list[1:3]
    pnet_bias2_fl = np.array(pnet_bias2).flatten() / 0.5
    pnet_bias2 = []
    for i in range(4):
    ...
    pnet_act2 = change_sram6_list[1:3]
    pnet_act2_fl = np.array(pnet_act2).flatten() / 4
    pnet_act2 = []
    for i in range(4):
    ...
    pnet_bias3 = change_sram6_list[3:7]
    ...
    pnet_act3 = change_sram6_list[3:7]
    pnet_act3_fl = np.array(pnet_act3).flatten() / 4
    pnet_act3 = []
    for i in range(8):
    ...
    pnet_bias4 = change_sram6_list[7:8]
    pnet_bias4_fl = np.array(pnet_bias4).flatten()
    pnet_bias4 = (pnet_bias4_fl / 128)[:2]
    pnet_bias5 = (pnet_bias4_fl / 32)[2:6]

    # rnet_bias1 = int2float(change_sram6_list[8:12], rnet_bias1_sh)
    rnet_bias1 = change_sram6_list[8:12]
    rnet_bias1_fl = np.array(rnet_bias1).flatten() / 4
    rnet_bias1 = []
    for i in range(7):
    ...
    rnet_act1 = change_sram6_list[8:12]
    ...
    rnet_bias2 = change_sram6_list[12:18]
    rnet_bias2_fl = np.array(rnet_bias2).flatten() / 4
    rnet_bias2 = []
    for i in range(12):
    ...
    rnet_act2 = change_sram6_list[12:18]
    rnet_act2_fl = np.array(rnet_act2).flatten() / 2
    rnet_act2 = []
    for i in range(12):
    ...
    rnet_bias3 = change_sram6_list[18:26]
    rnet_bias3_fl = np.array(rnet_bias3).flatten() / 4
    rnet_bias3 = []
    for i in range(16):
    ...
    rnet_act3 = change_sram6_list[18:26]
    rnet_act3_fl = np.array(rnet_act3).flatten() / 2
    rnet_act3 = []
    for i in range(16):
    ...
    rnet_bias4 = change_sram6_list[26:42]
    rnet_bias4_fl = np.array(rnet_bias4).flatten() / 4
    rnet_bias4 = []
    for i in range(128):
    ...
    rnet_act4 = change_sram6_list[26:42]
    rnet_act4_fl = np.array(rnet_act4).flatten() / 2
    rnet_act4 = []
    for i in range(128):
    ...
    rnet_bias5 = change_sram6_list[42:43]
    rnet_bias5_fl = np.array(rnet_bias5).flatten()
    rnet_bias5 = np.array((rnet_bias5_fl / 16)[:2]).flatten()
    rnet_bias6 = np.array((rnet_bias5_fl / 8)[2:6]).flatten()
